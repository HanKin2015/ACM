...
#    result_gnb.to_csv("result_gnb.csv",index=False)
    ...
    clf=svm.SVC(kernel='linear') #调用svm函数，并设置kernel参数，默认是rbf，其它：‘linear’‘poly’‘sigmoid’
#    result = []
#    for i in range(5):
...
#    print(result)
    ...
    print(tmp)
    passengers = DataFrame(passengers,columns=['PassengerId'])
    answer_svm = DataFrame(answer_svm,columns=['Survived'])
    result_svm = passengers.join(answer_svm)
    result_svm.to_csv("result_svm.csv",index=False)
